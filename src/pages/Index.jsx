...
const Index = () => {
  ...
  // New: refs and helpers for import/export/share
  const fileInputRef = useRef(null);

  const buildFormDataObject = () => ({
    billTo,
    shipTo,
    invoice,
    yourCompany,
  ...
  const exportJSON = () => {
    const dataStr = JSON.stringify(buildFormDataObject(), null, 2);
    const blob = new Blob([dataStr], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `billify-${invoice.number || 'invoice'}.json`;
    a.click();
  ...
  const onImportFile = (e) => {
    ...
    reader.onload = () => {
      try {
        ...
        setInvoice(data.invoice || { date: '', paymentDate: '', number: generateRandomInvoiceNumber() });
        setYourCompany(data.yourCompany || { name: '', address: '', phone: '' });
  ...
  const shareLink = async () => {
    const encoded = encodeURIComponent(btoa(unescape(encodeURIComponent(JSON.stringify(buildFormDataObject())))));
  ...
  useEffect(() => {
    ...
    if (encoded) {
      try {
        ...
        setBillTo(json.billTo || { name: "", address: "", phone: "" });
        setShipTo(json.shipTo || { name: "", address: "", phone: "" });
        setInvoice(json.invoice || { date: "", paymentDate: "", number: generateRandomInvoiceNumber() });
        setYourCompany(json.yourCompany || { name: "", address: "", phone: "" });
        setItems(Array.isArray(json.items) ? json.items : []);
        settaxPercentage(json.taxPercentage || 0);
        setNotes(json.notes || "");
        setSelectedCurrency(json.selectedCurrency || "INR");
        // Clean URL to avoid re-parsing
        window.history.replaceState({}, document.title, window.location.pathname);
        return;
      } catch (e) {
        console.warn('Failed to parse shared data param', e);
      }
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8 relative" data-scroll-section>
      <h1
        className="font-retro text-2xl sm:text-3xl md:text-4xl text-center mb-8 text-slate-900 dark:text-slate-100"
        data-scroll
        data-scroll-speed="2"
      >
        BILLIFY <span className="text-green-500">GENERATOR</span>
      </h1>
      <div className="fixed top-4 left-4 flex gap-2 z-10">
        <button
          onClick={clearForm}
          className="bg-red-500 text-white p-2 rounded-full shadow-lg hover:bg-red-600 active:scale-95 transition-transform focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400"
        ...
        <button
          ...
          className="bg-blue-500 text-white p-2 rounded-full shadow-lg hover:bg-blue-600 active:scale-95 transition-transform focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400"
      ...
      <button
        ...
        className="fixed top-4 right-4 bg-green-500 text-white p-2 rounded-full shadow-lg hover:bg-green-600 active:scale-95 transition-transform focus:outline-none focus-visible:ring-2 focus-visible:ring-green-400 z-10"
      ...
      <div className="flex flex-col md:flex-row gap-8">
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {[0, 5, 12, 18, 28].map((v) => (
                  <button
                    key={v}
                    type="button"
                    onClick={() => settaxPercentage(v)}
                    className={`px-2 py-1 rounded border ${taxPercentage === v ? 'bg-slate-900 text-white' : 'bg-white hover:bg-slate-100'}`}
                  >
                    {v}%
                  </button>
                ))}
        ...
        >
          <h2
            className="text-xl sm:text-2xl font-semibold mb-4 font-retro"
            data-scroll
            data-scroll-speed="1"
          >
            TEMPLATES
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {templates.map((template, index) => (
              <div
                key={index}
                className="template-card bg-gray-100 p-4 rounded-lg cursor-pointer hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 border border-gray-200"
                data-scroll
              ...
              >
                <img
                  ...
                  onLoad={() => window.__loco && window.__loco.update()}
                />
                <p className="text-center font-medium text-slate-700">{template.name}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Floating actions: Import / Export / Share */}
      <div className="fixed bottom-4 right-4 flex gap-2 z-10" data-scroll>
        <input
          type="file"
        ...
        <button
          ...
          className="bg-slate-800 text-white px-3 py-2 rounded-full shadow hover:bg-slate-700 active:scale-95 transition"
        ...
        <button
          onClick={shareLink}
          className="bg-green-600 text-white px-3 py-2 rounded-full shadow hover:bg-green-500 active:scale-95 transition"
